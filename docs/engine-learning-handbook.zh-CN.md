...
本文面向“能读 Python，但看到几千行 Agent 引擎不知道从哪里下手”的读者。依据本地提交 `0898459` 的源码编写；`coworker/engine.py` 共 2,879 行。文中的行号是此版本的导航，后续变更时优先搜索函数名。

学习目标：能独立解释一次用户输入如何经过模型、权限、工具和历史记录，最终得到回答。第一遍只读第 1～5 节；异常恢复和产品功能留到第二遍。

## 1. 先把 2,879 行缩成一张地图

真正反复执行的函数是 `_loop()`（632～864 行，约 230 行）。其余代码主要负责准备输入、工具授权、执行、交互等待、恢复和展示。

| 阅读层次 | 源码位置（engine.py 行号） | 要回答的问题 |
|---|---|---|
| 第一遍 | `run()`，397；`_loop()`，632 | 输入怎样进入？为什么会再次调用模型？ |
| 第一遍 | `_handle_tool_calls()`，1066 | 工具请求怎样变成实际操作？ |
| 第一遍 | `_execute_sync()`，1941；`_record_result()`，1954 | 谁调用 Python 函数？结果怎样送回模型？ |
| 第二遍 | `_astream()`，1017；`_outbound_messages()`，2601 | 流式响应怎样接入？发给模型的历史是什么？ |
| 第二遍 | `_authorize()`，1555 | 允许、拒绝、人工审批怎样改变执行？ |
| 按问题读 | `_compact_now()`，916；`retry()`，535；`resume()`，548 | 上下文变长、请求失败、重启后怎么继续？ |
| 暂时跳过 | `_handle_*` 交互处理器，2102～2586 | 团队、计划、连接器、目录请求怎样接入产品？ |
...
6. 为什么不能从第 1 行硬读到第 2,879 行？很多代码是特定产品交互和恢复分支，先沿一条执行路径读更容易建立因果关系。
